/simplify 审查并修复 the entire anyplane repository：覆盖 server/、web/、scripts/ 下全部 TypeScript/TSX 源码。不要只看最近 diff 或当前分支 pending changes——当前分支是新开的、没有 diff，必须主动把全仓库当作审查对象。按复用、简化、效率、altitude 清理标准执行并应用修复。
