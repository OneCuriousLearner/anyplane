...
- **依赖红线已由 Biome 机械执行**（biome.jsonc 的 noRestrictedImports，报错文案即规则意图）：适配器↛hub 运行时（hub/types 纯类型豁免）、hub↛push、routes↛具体后端实现（port/runtime/discovery/backend；routes 测试因注册真实适配器豁免 `*.test.ts`）、protocol 不出包。例外要进豁免清单并写明原因，不许静默绕过。时序红线（ensure 零 await）仍只有注释守护——GritQL 插件够不到 class 方法（Biome 2.5 限制）。
