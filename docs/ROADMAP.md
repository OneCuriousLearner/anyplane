...
3. ~~**客户端 attach 对齐**~~ ✅ 已做（2026-09-19，随 13.4 批次 A）：`SessionState`
   新增 `pendingApprovalIds` 快照（服务端 pending 唯一权威），客户端 status 到达即
   reconcile——只删快照外的卡、保留快照内的卡不 remount（盲清空会丢进行中选择，
   PR #50 review 发现一）；e2e-mock 补「断线错过 resolved」用例
...
修复单列排期。注意 verify 没有豁免机制：**修复前本机 `bun run verify` 预期在 test 步
失败（这 4 例），提交判定以 CI 为准**。
...
> 搬家改名（archive / vapid / lineage / streamJson）见 [delivered.md](delivered.md)。
> 下一层（非机械抽出）见本节「待排期」。
...
> **批次进度**：批次 A（审批链路对齐，2026-09-19）✅——`pendingApprovalIds` 快照 +
> 客户端 reconcile（`lib/approvals.ts`）+「断线错过 resolved」e2e 用例。**持久化缺口的结论修正**：重启后审批悬挂的正确解法是
...
**规划已沉淀（2026-09-19，docs/plans/agent-event-model.md）**：现状精确盘点
（13.1/13.3 后形状已中立、词汇表仍 vendor-anchored）、AgentEvent 判别联合草案、
四阶段迁移路径（定义 → 双发 → 前端切换 → 退役）、触发信号与风险清单。
**规划不是动工**：生产路径一行未改，阶段 0 的启动等触发信号。前置依赖：阶段 2 的
渲染链路对照验收需要 fixture 全绿——fixture S3 翻页断言存量失败须先修复（方向七段落）。
...
### 待排期（AI 维护残留的下一层，非机械抽出）

机械两档已收口（指针/分发/巨型文件/同形映射）。下面几条改的是语义或时机，**不要再开一档「再拆一次列表」**。
推进时各自独立成 PR，勿绑进 13.5。

| 项 | 现状锚点 | 何时动 | 不要做什么 |
|---|---|---|---|
| **processManager 下沉** | `claude/processManager.ts` 仍是会话类上帝文件（~860 行）。头部窗口启发式 / 用量口径 / 尾部解析 / 命令解析与会话生命周期无关，可先抽纯函数 | 下次改 spawn/回收/busy 启发式时顺手 | 不要借机重写 ClaudeSession；Windows 不再包 `cmd.exe`（旧包装已拆，见文件头） |
| **两套摘要口径** | 审批/推送正本是 `util.summarizeInput`（规则引擎字段对齐）；工具卡一行是 `web/lib/blocks.ts` `toolSummary`。用途不同 | 有第三个消费方或字段漂移时再合并 | 不要「为了同形」把审批摘要改成工具卡文案，或反过来 |
...
- **13.5 AgentEvent**：本节上一小节；规划 `docs/plans/agent-event-model.md`。前置：方向七 fixture S3 翻页断言须先绿。
- **单租户 / `homedir()`**：上一小节「明确不在本方向内」。
- **SessionList 主题长按 / 回收站视图**：仍在 `pages/SessionList.tsx`（本体已 476 行）。再抽收益低于上表，触碰该页时再看。
