...
> **已落地（2026-09-19）**：面板纯函数 + 单测（2026-08-27）；分区表与 `conversation_reset` 不变量单测（PR #58）。后续机械清理未再改斜杠语义。
> **仍有效**：claude 未知斜杠 / codex 必须拦截、/clear 重键、/goal 跟踪、codex 拦截表双处同步。新增命令先对照本文。
